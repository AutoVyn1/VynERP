...
Imports System.IO
    ...
    Public Class Ledg_dashboard
        ...
        TranDt3 = con.ReturnDtTable("SELECT YEAR(Acnt_Date) AS Year, 'Q ' + CAST(DATEPART(QUARTER, Acnt_Date) AS VARCHAR) AS Quarter, cast(sum(iif(amt_drcr=1,post_amt,post_amt*-1))/100000 AS VARCHAR) + ' L' AS cl_bal FROM acnt_post WHERE ledg_ac = '" + grp_name + "' and Acnt_Date between '04/01/" + frm_year + "' and '03/31/" + to_year + "' GROUP BY YEAR(Acnt_Date), 'Q ' + CAST(DATEPART(QUARTER, Acnt_Date) AS VARCHAR) ORDER BY Year, Quarter")
        ...
        Dim TranDt3 As DataTable
        TranDt3 = con.ReturnDtTable("SELECT YEAR(Acnt_Date) AS Year, 'Q ' + CAST(DATEPART(QUARTER, Acnt_Date) AS VARCHAR) AS Quarter,  cast(sum(iif(amt_drcr=1,post_amt,post_amt*-1))/100000 AS VARCHAR) + ' L' AS cl_bal FROM acnt_post WHERE ledg_ac = '" + grp_name + "' and Acnt_Date between '04/01/" + frm_year + "' and '03/31/" + to_year + "'  and Loc_Code ='" + loc_code + "' GROUP BY YEAR(Acnt_Date), 'Q ' + CAST(DATEPART(QUARTER, Acnt_Date) AS VARCHAR) ORDER BY Year, Quarter")
        ...
        Public Shared Function GetChartData2(grp_name As String, frm_year As String, to_year As String, xValue As String) As String
            ...
            Dim TranDt As DataTable
        TranDt = con.ReturnDtTable("select 'Month '+cast(Month(Acnt_Date)AS VARCHAR) as Month, cast(sum(iif(amt_drcr=1,post_amt,post_amt*-1))/100000 AS VARCHAR) + ' L' AS cl_bal from acnt_post  where ledg_ac = '" + grp_name + "' and Acnt_Date between '" + datew + "' " + loc_code + "  group by month(Acnt_Date) order by month(Acnt_Date)")
